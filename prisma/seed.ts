/**
 * 种子脚本：仅写入预设账号 FD001–FD020 与平台基础配置。
 * 不导入样例委托人 / 设计师 / 订单 / 悬赏等业务数据。
 * 运行：npm run db:seed 或 npm run prod:db:seed
 *
 * 预设账号：
 *   FD001 超管 FD19076652
 *   FD002–FD004 管理员 FD4006801231
 *   FD005–FD020 普通账号 4006801231（登录后自行注册委托人/设计师）
 */
...
import { DEFAULT_SERVICE_PROVIDERS } from "../src/lib/service-provider-catalog";
...
async function main() {
  ...
  for (const provider of DEFAULT_SERVICE_PROVIDERS) {
