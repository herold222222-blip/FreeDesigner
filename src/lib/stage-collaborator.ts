...
/** 合并订单数据与运行时 store 中的配合服务记录 */
export function getMergedStageCollaborators(
  order: Order,
  stageId: string,
): StageCollaboratorService[] {
  const persisted = order.stageCollaborators?.filter((c) => c.stageId === stageId) ?? [];
...
export function getMergedOrderCollaborators(order: Order): StageCollaboratorService[] {
  return order.stages.flatMap((stage) => getMergedStageCollaborators(order, stage.id));
}
